...
import { EnhancedProductFormSchema } from '../schemas'
...
export async function createProduct(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (category?.type === 'COFFEE' && coffeeCharacteristics) {
      ...
      } else if (category?.type === 'EQUIPMENT' && equipmentSpecs) {
        await tx.equipmentSpecs.create({
          data: {
            ...equipmentSpecs,
            productId: product.id,
          },
        })
      } else if (category?.type === 'ACCESSORY' && accessorySpecs) {
...
export async function editProduct(
  ...
  try {
    ...
    if (!category) {
      return {
        errors: {
          _form: ['دسته‌بندی انتخاب شده موجود نیست!'],
        },
      }
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Delete existing characteristics first
      if (isExisting.coffeeCharacteristics) {
        await tx.coffeeCharacteristics.delete({
          where: { productId },
        })
      }
      if (isExisting.equipmentSpecs) {
        await tx.equipmentSpecs.delete({
          where: { productId },
        })
      }
      if (isExisting.accessorySpecs) {
        await tx.accessorySpecs.delete({
          where: { productId },
        })
      }
