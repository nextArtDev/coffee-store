...
import { Billboard, Image } from '@/lib/generated/prisma'
...
import { createBillboard, editBillboard } from '../../../lib/actions/billboard'

interface BillboardDetailsProps {
  initialData?: Billboard & { images: Image[] }
}

const BillboardDetails: FC<BillboardDetailsProps> = ({ initialData }) => {
  ...
  const form = useForm<z.infer<typeof BillboardFormSchema>>({
    ...
    resolver: zodResolver(BillboardFormSchema), // Resolver for form validation
    defaultValues: {
  ...
  const handleSubmit = (data: z.infer<typeof BillboardFormSchema>) => {
    startTransition(async () => {
      try {
        if (initialData) {
          const res = await editBillboard(data, initialData.id, path)
          if (res?.errors) handleServerErrors(res.errors, form.setError)
        } else {
          const res = await createBillboard(data, path)
          if (res?.errors) handleServerErrors(res.errors, form.setError)
        }
      } catch (error) {
  ...
  const billboardName = form.watch('name')

  useEffect(() => {
    if (!isUrlManuallyEdited && billboardName) {
      form.setValue('url', slugify(billboardName), { shouldValidate: true })
    }
  }, [billboardName, isUrlManuallyEdited, form])
  return (
    <AlertDialog>
