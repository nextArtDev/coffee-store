...
interface DualOrbitCarouselProps {
  categories: CategoryWithStats[]
}
// export default function DualOrbitCarousel() {
const DualOrbitCarousel: FC<DualOrbitCarouselProps> = ({ categories }) => {
  const [activeGroupIndex, setActiveGroupIndex] = useState(0)
  const [isHovering, setIsHovering] = useState(false) // For pausing carousel
  const isMobile = useIsMobile()

  const activeGroup = useMemo(
    () => categories[activeGroupIndex],
    [activeGroupIndex, categories]
  )
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setActiveGroupIndex((prev) => (prev + 1) % categories.length)
    ...
    >
      ...
      >
        ...
        <AnimatePresence>
          ...
          >
            {activeGroup?.subCategories.map((member, i) => {
              const angle = (i / activeGroup.subCategories.length) * 360
              return (
                <div
                  key={member.id}
                ...
                >
                  ...
                  <motion.img
                    src={member.images.map((img) => img.url)[0]}
                    alt={member.name}
                    onError={safeImage}
            ...
            return (
              ...
              >
                ...
                <motion.div
                  ...
                  className="flex items-center justify-center rounded-full cursor-pointer bg-transparent dark:bg-gray-800 text-indigo-500 dark:text-indigo-400  shadow-lg overflow-hidden"
                  style={{
                    width: groupSize,
                    height: groupSize,
                  }}
                ...
                >
                  {/* {group.} */}
                  <motion.img
                    src={group.images.map((img) => img.url)[0]}
                    alt={group.name}
                    onError={safeImage}
                    // className="object-cover rounded-full shadow-md border-2 border-gray-300 dark:border-gray-600"
                    className="object-cover rounded-full shadow-md  "
...
export default DualOrbitCarousel
